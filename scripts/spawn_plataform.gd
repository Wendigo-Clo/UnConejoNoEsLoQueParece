...
@export var distancia_spawn = 150  # Distancia vertical mínima entre plataformas
...
func _ready() -> void:
	last_spawn_y = get_viewport().get_camera_2d().global_position.y + distancia_spawn
	ultimoFue = plataform
	spawnear()
...
func spawnear():
	...
	if(plataform and Global.sigueVivo):
